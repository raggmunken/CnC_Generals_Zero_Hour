...
Kimi: AIRCRAFT + AI AIR PLAY + DEPLOY PACKAGE. The Attack Chopper is in: air
armour, rocket armament, built at the War Factory. Air units skip pathfinding
entirely (the sky has no walls), fly straight over mountains/water/bases with
only a map-edge clamp, and never collide across altitudes in separation.
Client draws them hovering 0.6 above their tile with the shadow and selection
ring left on the ground. Cannons and artillery score a genuine zero against
air, so air forces flak -- and the AI now plays both sides of that: choppers
are kept at a quarter of its vehicle fleet (fleet-ratio rule, so the first
vehicle it fields is one), and its existing enemy model already queues AA
vehicles and AA batteries the moment it SEES enemy air. Audit: 44 working,
0 broken, 2 not implemented (was 38/0/3; aircraft left the gap list and six
new checks landed, incl. "AI answers aircraft with flak" which parks a chopper
in the AI's vision and watches its queues respond). Self-play mirror still
settles at 50% [15%,85%], so the harness self-check holds with the new mix.
